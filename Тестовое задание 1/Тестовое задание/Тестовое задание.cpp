// Тестовое задание.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
// -------------------------------------------------------------------------------------------------------------- //
// 
// ЗАДАНИЕ
//
//Распечатать произвольное число в двоичной системе исчисления.
//
//Описание.
//Написать функцию, принимающую на вход произвольное целочисленное значение и возвращающую строку, содержащую его представление в двоичной системе исчисления.
//
//Декларация.
//voidNumberAsBinary(
//    char* _result,// строка с результатом
//    unsignedint _number// произвольное число
//)
//
//Ожидаемый результат.
//Например, для числа 13 данная функция должна вернуть строку "1101".
//
//Проверка работы.
//Решение должно демонстрировать работу функции на примере 10 случайных чисел в диапазоне 0 до 512.
//





#include <iostream>
#include <bitset>
#include <stdlib.h>
#include <time.h>
#include <string>


void NumberAsBinary(char* _result, unsigned int _number);

int main() {
    //unsigned int number = 123;
    
    char binary[33]; // Максимальная длина представления в двоичном виде для 32-битного числа + символ конца строки


    srand(time(0));

    for (size_t i = 0; i < 10; i++)
    {
        int num = 0 + rand() % (512 - 0 + 1);
        NumberAsBinary(binary, num);
        std::cout << binary << std::endl;
    }

    return 0;
}

void NumberAsBinary(char* _result, unsigned int _number) {
    for (int i = 31; i >= 0; --i) {
        _result[i] = (_number & 1) ? '1' : '0';
        _number >>= 1;
    }
    _result[32] = '\0'; // Добавляем символ конца строки
}
// Запуск программы: CTRL+F5 или меню "Отладка" > "Запуск без отладки"
// Отладка программы: F5 или меню "Отладка" > "Запустить отладку"

// Советы по началу работы 
//   1. В окне обозревателя решений можно добавлять файлы и управлять ими.
//   2. В окне Team Explorer можно подключиться к системе управления версиями.
//   3. В окне "Выходные данные" можно просматривать выходные данные сборки и другие сообщения.
//   4. В окне "Список ошибок" можно просматривать ошибки.
//   5. Последовательно выберите пункты меню "Проект" > "Добавить новый элемент", чтобы создать файлы кода, или "Проект" > "Добавить существующий элемент", чтобы добавить в проект существующие файлы кода.
//   6. Чтобы снова открыть этот проект позже, выберите пункты меню "Файл" > "Открыть" > "Проект" и выберите SLN-файл.
